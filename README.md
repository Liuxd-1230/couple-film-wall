...
3. 在 SQL Editor 执行 `supabase/migrations/20260707000000_initial_schema.sql`。
4. 两个人都用邮箱登录一次后，修改并执行 `supabase/seed-members.sql`，把两个人加入同一个 `couple_id`。
5. 前端环境变量使用 publishable key 或 anon key，不要使用 service role key。
